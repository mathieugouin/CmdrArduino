...
DCCPacket::DCCPacket(uint16_t new_address, uint8_t new_address_kind) :
  address(new_address), address_kind(new_address_kind), size_repeat(0x40), kind(idle_packet_kind) //size(1), repeat(0)
{
  address = new_address;
...
uint8_t DCCPacket::getBitstream(uint8_t rawbytes[]) //returns size of array.
{
  ...
  else if (kind & ACCESSORY_PACKET_KIND_MASK)
  {
    if (kind == basic_accessory_packet_kind)
    {
      ...
      rawbytes[1] |= (~(address >> 2) & 0x70) | (data[0] & 0x07);
